...
Each component has a standalone `test_*.py` script at the project root (no pytest framework yet — these are direct sanity checks with assertions, runnable individually). The first 9 use fake embeddings / the keyword classifier fallback / monkeypatching and need no API key; the last 2 make real OpenAI calls:

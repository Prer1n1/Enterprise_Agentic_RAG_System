...
curl -F "file=@/path/to/your/document.pdf" http://127.0.0.1:8000/documents/upload
...
curl -X POST http://127.0.0.1:8000/ingest/drive
```
...
Built so far: Ingestion & Processing, Storage, Retrieval, Agent Orchestration (LangGraph), Evaluation & Observability (RAGAS, hallucination detection, LangSmith tracing), API Layer (FastAPI), Docker packaging.
Not yet built: authentication, structured logging/CI.

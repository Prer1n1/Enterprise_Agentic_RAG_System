...
- **Known limitation surfaced by this same real data, deliberately not fixed yet**: the SEC investor bulletin's 26 chunks were classified `{'Legal': 17, 'General': 6, 'HR': 3}` — zero tagged **Finance**, despite the document being entirely about investing. The rule-based keyword classifier (see Ingestion & Processing above) was tuned against this project's synthetic sample data, whose Finance vocabulary (expense, reimbursement, budget) doesn't overlap with real SEC regulatory language (offering, prospectus, securities, issuer). This doesn't break retrieval (hybrid search still finds the content by keyword/semantic match regardless of its category tag), but it does break category-filtered search ("show me only Finance docs" would miss this document entirely). Real evidence that a classifier validated only on synthetic data doesn't generalize — left as a documented known limitation rather than patched reactively, since the fix (e.g., LLM-based zero-shot classification) is a deliberate scoped follow-up, not a quick patch.
